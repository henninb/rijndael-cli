...
public class PasswdSetup {
  ...
  private static byte[] getPasswd() {
    ...
    int keySizeStatus = chooseOne("Choose a encryption KEY size", "128", "256");
    int keyBytes = (keySizeStatus == 0) ? 16 : 32;

    byte[] salt = new byte[16];
    RandomNumberGenerator.Fill(salt);

    KEY = Rfc2898DeriveBytes.Pbkdf2(passwd, salt, 200_000, HashAlgorithmName.SHA512, keyBytes);

    fileWrite("input.salt", salt);
    Console.WriteLine("PBKDF2-HMAC-SHA512({0}-bit, 200000 iterations) key derived.", keyBytes * 8);
