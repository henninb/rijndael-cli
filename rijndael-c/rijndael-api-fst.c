...
int makeKey( keyInstance *key, BYTE direction, int keyLen, unsigned char *keyMaterial ) {
  ...
  if( keyMaterial != NULL ) {
    /* Fix #8: strncpy does not guarantee null terminator when src fills the buffer */
...
int blockEncrypt( cipherInstance *cipher, keyInstance *key, BYTE *input, int inputLen, BYTE *outBuffer ) {
  ...
  case MODE_CBC:
    ...
    for( i = numBlocks; i > 0; i-- ) {
      /* Fix #9: byte loop avoids u8*→u32* strict aliasing violation */
      for( j = 0; j < 16; j++ ) block[j] = input[j] ^ iv[j];
      //printf("key->Nr = %d\n", key->Nr);
...
int blockDecrypt( cipherInstance *cipher, keyInstance *key, BYTE *input, int inputLen, BYTE *outBuffer ) {
  ...
  /* Fix #2: original had &&, causing key->direction deref when key==NULL;
     also the direction check was inverted (should reject DIR_ENCRYPT keys) */
